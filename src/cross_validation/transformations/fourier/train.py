...
save_path = f"results/cross_validation/transformations/fourier"
data_path = "data/processed/mcf10a/high_dose/train"

data = load_data(data_path)

data[0] = pd.DataFrame(np.abs(np.fft.fft(data[0], axis=1)))
data[1] = pd.DataFrame(np.abs(np.fft.fft(data[1], axis=1)))
